...
The example also showcases a resilient approach to entity onboarding: check `EntityTokenOfAccount` first, reuse the token if it exists, and only call `set_info` when the storage entry is empty. This keeps developer nodes tidy while still exercising the pallet logic whenever a fresh account is used.
