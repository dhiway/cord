...
CORD derives every on-chain identifier from pallet-specific material and the global SS58 checksum rules. Each identifier is encoded as a Base58 string so that humans can visually compare and copy them with minimal friction. In the Subxt anchor example (`examples/subxt-anchor/src/flows.rs`) we call `Entity::set_info` whenever the signer does not already own an entity token. The emitted `EntityInfoSet` event includes the freshly minted `Ss58Identifier`, which we render inside the walkthrough table so developers can immediately see which pallet produced the identifier.
