...
```rust
use oc::demo::util::fresh_authorization; // thin wrapper

let auth = fresh_authorization(&signer)?; // random "cord:view:v1" payload
```

Under the hood this calls `AuthorizationBuilder::from_signer(&keypair, None)`, detects the signature scheme, and returns an `AuthorizationRequest` ready for the view functions.
...
```rust
let req = RegisterDetailsRequest { auth: fresh_authorization(&signer)?, registry: registry_id.clone() };
