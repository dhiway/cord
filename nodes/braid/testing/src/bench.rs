...
//! can pregenerate seed database and `clone` it for every iteration of your
//! benchmarks or tests to get consistent, smooth benchmark experience!
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		let signed = self.keyring.sign(
			CheckedExtrinsic {
				signed: Some((
					sender,
					signed_extra(0, cord_braid_runtime::ExistentialDeposit::get() + 1),
				)),
				function: match self.content.block_type {
					BlockType::RandomTransfersKeepAlive => {
						RuntimeCall::Balances(BalancesCall::transfer_keep_alive {
							dest: sp_runtime::MultiAddress::Id(receiver),
							value: cord_braid_runtime::ExistentialDeposit::get() + 1,
						})
					},
					BlockType::RandomTransfersReaping => {
						RuntimeCall::Balances(BalancesCall::transfer_allow_death {
							...
							value: 100 * WAY - (cord_braid_runtime::ExistentialDeposit::get() - 1),
						})
					},
					BlockType::Noop => {
...
impl BenchKeyring {
	...
	/// balance and cord_braid_runtime code blob.
...
impl sp_runtime::BuildStorage for BenchKeyring {
	fn assimilate_storage(&self, storage: &mut sp_core::storage::Storage) -> Result<(), String> {
		storage.top.insert(
			...
			cord_braid_runtime::wasm_binary_unwrap().into(),
		);
