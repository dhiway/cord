...
fn version_is_full() {
	...
	let output = Command::new(cargo_bin("braid")).args(["--version"]).output().unwrap();
...
fn test_regex_matches_properly() {
	...
	let captures = expected.captures("braid 0.7.0-da487d19d").unwrap();
	assert_eq!(&captures[1], "0.7.0");
	assert_eq!(&captures[2], "da487d19d");

	let captures = expected.captures("braid 0.7.0-alpha.5-da487d19d").unwrap();
