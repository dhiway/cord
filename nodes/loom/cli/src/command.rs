...
use cord_loom_runtime::{ExistentialDeposit, RuntimeApi};
...
impl SubstrateCli for Cli {
	...
	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		let spec = match id {
			"loom" | "" => Box::new(chain_spec::loom_local_config()?),
			"dev" | "loom-dev" => Box::new(chain_spec::loom_dev_config()?),
			"local" | "loom-local" => Box::new(chain_spec::loom_local_config()?),
			path => {
				let path = std::path::PathBuf::from(path);
				let chain_spec = Box::new(chain_spec::CordChainSpec::from_json_file(path.clone())?)
					as Box<dyn sc_service::ChainSpec>;
				if chain_spec.is_cord()
					|| chain_spec.is_loom_local()
					|| chain_spec.is_loom_staging()
				{
