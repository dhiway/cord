...
use crate::cli::Cli;
use codec::Encode;
pub use cord_loom_runtime::RuntimeApi;
...
impl IdentifyVariant for Box<dyn ChainSpec> {
	...
	fn identify_chain(&self) -> Chain {
		if self.is_cord() || self.is_loom_local() || self.is_loom_local() || self.is_loom_staging()
		{
			Chain::Loom
		} else {
			Chain::Unknown
...
pub fn create_extrinsic(
	...
	function: impl Into<cord_loom_runtime::RuntimeCall>,
	nonce: Option<u32>,
) -> cord_loom_runtime::UncheckedExtrinsic {
...
exists; qed",
	...
	let period = cord_runtime_common::BlockHashCount::get()
		...
		.unwrap_or(2) as u64;
	let tip = 0;
	let extra: cord_loom_runtime::SignedExtra = (
		pallet_network_membership::CheckNetworkMembership::<cord_loom_runtime::Runtime>::new(),
		frame_system::CheckNonZeroSender::<cord_loom_runtime::Runtime>::new(),
		frame_system::CheckSpecVersion::<cord_loom_runtime::Runtime>::new(),
		frame_system::CheckTxVersion::<cord_loom_runtime::Runtime>::new(),
		frame_system::CheckGenesis::<cord_loom_runtime::Runtime>::new(),
		frame_system::CheckMortality::<cord_loom_runtime::Runtime>::from(generic::Era::mortal(
			period,
			best_block.saturated_into(),
		)),
		frame_system::CheckNonce::<cord_loom_runtime::Runtime>::from(nonce),
		frame_system::CheckWeight::<cord_loom_runtime::Runtime>::new(),
		pallet_transaction_payment::ChargeTransactionPayment::<cord_loom_runtime::Runtime>::from(
			tip,
		),
	);

	let raw_payload = cord_loom_runtime::SignedPayload::from_raw(
		function.clone(),
		extra.clone(),
		(
			(),
			(),
			cord_loom_runtime::VERSION.spec_version,
			cord_loom_runtime::VERSION.transaction_version,
			genesis_hash,
			best_hash,
			(),
			(),
	...
	cord_loom_runtime::UncheckedExtrinsic::new_signed(
		function,
		sp_runtime::AccountId32::from(sender.public()).into(),
		cord_loom_runtime::Signature::Sr25519(signature),
		extra,
	)
}
...
> {
	...
	let (rpc_extensions_builder, rpc_setup) = {
		...
		let rpc_extensions_builder = move |deny_unsafe, subscription_executor| {
			let deps = cord_loom_node_rpc::FullDeps {
				client: client.clone(),
				pool: pool.clone(),
				select_chain: select_chain.clone(),
				chain_spec: chain_spec.cloned_box(),
				deny_unsafe,
				babe: cord_loom_node_rpc::BabeDeps {
					keystore: keystore.clone(),
					babe_worker_handle: babe_worker_handle.clone(),
				},
				grandpa: cord_loom_node_rpc::GrandpaDeps {
			...
			cord_loom_node_rpc::create_full(deps).map_err(Into::into)
		};
...
pub fn new_full_base<N: NetworkBackend<Block, <Block as BlockT>::Hash>>(
	...
	let backoff_authoring_blocks = if config.chain_spec.is_cord() {
		...
		if config.chain_spec.is_loom_local()
			|| config.chain_spec.is_loom_staging()
