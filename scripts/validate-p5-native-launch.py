...
MIGRATED_DOMAINS = ("attestation", "identity", "personhood", "individuality", "names", "storage")
...
CLEAN_BREAK_FORBIDDEN_SCOPES = {
    "identity:read", "attestation:read", "names:resolve", "storage:read",
...
def validate_host_and_operator(execute: bool) -> tuple[dict, dict, list[str]]:
    ...
    required_steps = {
        "verify-runtime-binding", "register-names-registrar", "register-attestation-schema",
...
def validate_cutover(execute: bool) -> tuple[dict, dict, list[str]]:
    ...
    if allowlist.get("schema_version") != 3:
        errors.append("Revive allowlist must use strict schema version 3")
    ...
    for path in files:
        ...
        if not rel.startswith(("docs/evidence/", "docs/architecture/", "docs/adr/")):
            ...
            if path.suffix.lower() in {".rs", ".ts", ".tsx", ".js", ".json", ".toml", ".md", ".yml", ".yaml", ".sh"} \
                ...
                content = path.read_text(errors="replace")
                if re.search(
                    r"(?is)(?:attestation|identity|personhood|individuality|names|storage).{0,120}"
                    ...
                    r"(?:attestation|identity|personhood|individuality|names|storage)",
                    content,
                ):
    ...
    migrated_revive_calls = re.findall(
        r"(?im)^.*(?:attestation|identity|personhood|individuality|names|storage).*(?:RuntimeCall::Revive|pallet_revive::Call|eth_substrate_call).*$",
        runtime_sources,
    )
    app = read_json(APP_MANIFEST)
    descriptor = read_json(DESCRIPTOR)
