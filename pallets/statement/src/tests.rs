...
fn test_statement_revoked() {
    ...
    let raw_schema = [11u8; 256].to_vec();
    let schema: InputSchemaOf<Test> = BoundedVec::try_from(raw_schema)
        .expect("Schema should fit into the expected input length for the test runtime.");
    let schema_id_digest = <Test as frame_system::Config>::Hashing::hash(
        &[&schema.encode()[..], &space_id.encode()[..], &creator.encode()[..]].concat()[..],
    );
    let schema_id: SchemaIdOf = generate_schema_id::<Test>(&schema_id_digest);
    ...
    new_test_ext().execute_with(|| {
        ...
        // Create schema and register statement
        assert_ok!(Schema::create(
            DoubleOrigin(author.clone(), creator.clone()).into(),
            schema.clone(),
            authorization_id.clone()
        ...
        // Attempt to update the revoked statement should fail
        assert_err!(
            Statement::update(
                DoubleOrigin(author.clone(), creator.clone()).into(),
                statement_id.clone(),
        ...
        // Attempt to revoke the already revoked statement should fail
        assert_err!(
            Statement::revoke(
                DoubleOrigin(author, creator).into(),
                statement_id,
