...
#[test]
fn registering_with_invalid_identifier_length_should_fail() {
	let creator = DID_00;
	let author = ACCOUNT_00;
	let capacity = 5u64;
	let statement = [77u8; 32];
	...
	new_test_ext().execute_with(|| {
		...
		let too_short_data = [0u8; 82];
		assert_err!(
			Ss58Identifier::create_identifier(&too_short_data, IdentifierType::Statement),
			identifier::IdentifierError::InvalidIdentifierLength
		);
