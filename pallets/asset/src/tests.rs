...
#[test]
fn asset_issue_by_a_non_delegate_should_fail() {
    let author = ACCOUNT_00;
    let creator = DID_00;
    let delegate = DID_01;
    let asset_desc = BoundedVec::try_from([72u8; 10].to_vec()).unwrap();
    ...
    let create_id_digest = <Test as frame_system::Config>::Hashing::hash(
        &[&digest.encode()[..], &space_id.encode()[..], &creator.encode()[..]].concat()[..],
    );

    let asset_id: Ss58Identifier = generate_asset_id::<Test>(&create_id_digest);
    ...
    new_test_ext().execute_with(|| {
        ...
        assert_err!(
            Asset::issue(
                DoubleOrigin(author.clone(), delegate).into(),
                issue_entry.clone(),
                issue_entry_digest,
                authorization_id
            ),
...
#[test]
fn asset_transfer_by_a_non_delegate_should_fail() {
    let author = ACCOUNT_00;
    let creator = DID_00;
    let receiver = DID_01;
    let asset_desc = BoundedVec::try_from([72u8; 10].to_vec()).unwrap();
    ...
    let issue_id_digest = <Test as frame_system::Config>::Hashing::hash(
        &[&asset_id.encode()[..], &creator.encode()[..], &space_id.encode()[..], &creator.encode()[..], &issue_entry_digest.encode()[..]].concat()[..],
    );

    let instance_id = generate_asset_instance_id::<Test>(&issue_id_digest);

    let transfer_entry = AssetTransferEntryOf::<Test> {
        asset_id: asset_id.clone(),
        asset_instance_id: instance_id.clone(),
        asset_owner: creator.clone(),
        new_asset_owner: receiver.clone(),
    };
    ...
    new_test_ext().execute_with(|| {
        ...
        assert_err!(
            Asset::transfer(
                DoubleOrigin(author.clone(), receiver.clone()).into(),
                transfer_entry,
...
#[test]
fn asset_status_change_by_a_non_delegate_should_fail() {
    let author = ACCOUNT_00;
    let creator = DID_00;
    let delegate = DID_01;
    let asset_desc = BoundedVec::try_from([72u8; 10].to_vec()).unwrap();
