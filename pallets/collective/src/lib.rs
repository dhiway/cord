...
//!
//! The membership can be provided in one of two ways: either directly, using
//! the Root-dispatchable function `set_members`, or indirectly, through
//! implementing the `ChangeMembers`. The pallet assumes that the amount of
//! members stays at or below `MaxMembers` for its weight calculations, but
//! enforces this neither in `set_members` nor in `change_members_sorted`.
//!
//! A "prime" member may be set to help determine the default vote behavior
//! based on chain config. If `PrimeDefaultVote` is used, the prime vote acts as
//! the default vote in case of any abstentions after the voting period. If
//! `MoreThanMajorityThenPrimeDefaultVote` is used, then abstentions will first
//! follow the majority of the collective voting, and then the prime member.
//!
//! Voting happens through motions comprising a proposal (i.e. a curried
//! dispatchable) plus a number of approvals required for it to pass and be
//! called. Motions are open for members to vote on for a minimum period given
//! by `MotionDuration`. As soon as the needed number of approvals is given, the
//! motion is closed and executed. If the number of approvals is not reached
//! during the voting period, then `close` may be called by any account in order
//! to force the end the motion explicitly. If a prime member is defined then
//! their vote is used in place of any abstentions and the proposal is executed
//! if there are enough approvals counting the new votes.
//!
//! If there are not, or if no prime is set, then the motion is dropped without
//! being executed.
...
/// First see if yes vote are over majority of the whole collective. If so, set
/// the default vote as yes. Otherwise, use the prime member's vote as the
/// default vote.
...
pub mod pallet {
		...
		frame_system::Config + pallet_network_membership::Config
	{
		...
		/// The maximum number of members supported by the pallet. Used for
		/// weight estimation.
		///
		/// NOTE:
...
impl<T: Config<I>, I: 'static> Pallet<T, I> {
	...
	/// Add an aye or nay vote for the member to the given proposal, returns
	/// true if it's the first vote of the member in the motion
	pub fn do_vote(
		who: T::AccountId,
		proposal: T::Hash,
		index: ProposalIndex,
...
impl<T: Config<I>, I: 'static> ChangeMembers<T::AccountId> for Pallet<T, I> {
	/// Update the members of the collective. Votes are updated and the prime is
	/// reset.
	///
...
/// Ensure that the origin `o` represents at least `n` members. Returns `Ok` or
/// an `Err` otherwise.
