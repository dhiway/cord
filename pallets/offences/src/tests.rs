...
fn should_report_an_authority_and_trigger_on_offence_and_add_to_blacklist() {
	new_test_ext().execute_with(|| {
		...
		let offence = Offence { validator_set_count: 5, time_slot, offenders: vec![5, 9] };
...
fn should_not_report_the_same_authority_twice_in_the_same_slot() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			offence_reports(KIND, time_slot),
			vec![OffenceDetails { offender: 5, reporters: vec![] },]
		);
	});
}
...
fn should_report_in_different_time_slot() {
	new_test_ext().execute_with(|| {
		...
		System::assert_last_event(
			Event::Offence { kind: KIND, timeslot: time_slot.encode() }.into(),
		);
		...
		// then
		System::assert_last_event(
			Event::Offence { kind: KIND, timeslot: offence.time_slot.encode() }.into(),
		);
	});
}
...
fn should_properly_count_offences() {
	// We report two different authorities for the same issue. Ultimately, the 1st
	// authority should have `count` equal 2 and the count of the 2nd one should be
	// equal to 1.
	new_test_ext().execute_with(|| {
		// given
		let time_slot = 42;
		...
		// then
		// the 1st authority should have count 2 and the 2nd one should be reported only
		// once.
		assert_eq!(
			offence_reports(KIND, time_slot),
			vec![
