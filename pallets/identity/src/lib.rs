...
//! * `request_verification` - Request a verification from a registrar, paying a fee.
//! * `cancel_request` - Cancel the previous request for a judgement.
//!
//! #### For general users with spaces (sub-identities)
//! * `set_spaces` - Set the spaces of an identity.
//! * `add_sub` - Add a space to an identity.
//! * `remove_sub` - Remove a space of an identity.
//! * `rename_sub` - Rename a space of an identity.
//! * `quit_sub` - Remove a sub-identity of an identity (called by the sub-identity).
//!
//! #### For registrars
//! * `set_fee` - Set the fee required to be paid for a judgement to be given by the registrar.
//! * `set_fields` - Set the fields that a registrar cares about in their judgements.
//! * `provide_verification` - Provide a verification response to an identity.
//!
//! #### For super-users
...
pub use pallet::*;
pub use types::{
	Data, IdentityField, IdentityFields, IdentityInfo, RegistrarIndex, RegistrarInfo, Registration, Verification,
};
...
pub mod pallet {
	...
	pub(super) type Registrars<T: Config> =
		StorageValue<_, BoundedVec<Option<RegistrarInfo<BalanceOf<T>, T::AccountId>>, T::MaxRegistrars>, ValueQuery>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn add_registrar(origin: OriginFor<T>, account: T::AccountId) -> DispatchResultWithPostInfo {
			...
			let (i, registrar_count) =
				<Registrars<T>>::try_mutate(|registrars| -> Result<(RegistrarIndex, usize), DispatchError> {
					registrars
		...
		pub fn request_judgement(
			...
			Ok(Some(T::WeightInfo::request_judgement(judgements as u32, extra_fields as u32)).into())
		}
		...
		pub fn provide_judgement(
			...
			Ok(Some(T::WeightInfo::provide_judgement(judgements as u32, extra_fields as u32)).into())
		}
