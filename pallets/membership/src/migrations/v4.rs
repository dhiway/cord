...
///
/// This new prefix must be the same as the one set in construct_runtime. For
/// safety, use `PalletInfo` to get it, as:
/// `<Runtime as frame_system::Config>::PalletInfo::name::<MembershipPallet>`.
///
/// The migration will look into the storage version in order not to trigger a
/// migration on an up to date storage. Thus the on chain storage version must
/// be less than 4 in order to trigger the migration.
...
/// [`frame_support::traits::OnRuntimeUpgrade::pre_upgrade`] for further
/// testing.
///
...
pub fn pre_migrate<P: GetStorageVersion, N: AsRef<str>>(old_pallet_name: N, new_pallet_name: N) {
	...
	// Ensure nothing except maybe the storage_version_key is stored in the new
	// prefix.
...
/// [`frame_support::traits::OnRuntimeUpgrade::post_upgrade`] for further
/// testing.
///
/// Panics if anything goes wrong.
