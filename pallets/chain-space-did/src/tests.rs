...
fn add_delegate_should_succeed() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_ok!(SpaceDid::add_delegate(
			DoubleOrigin(author.clone(), creator.clone()).into(),
			space_id,
			DID_01,
			authorization_id,
...
fn add_admin_delegate_should_succeed() {
	...
	new_test_ext().execute_with(|| {
		...
		//Admin should be able to add the delegate
		assert_ok!(SpaceDid::add_admin_delegate(
			DoubleOrigin(author.clone(), creator.clone()).into(),
			space_id,
			DID_01,
			authorization_id,
...
fn add_delegator_should_succeed() {
	...
	new_test_ext().execute_with(|| {
		...
		//Admin should be able to add the delegate
		assert_ok!(SpaceDid::add_delegator(
			DoubleOrigin(author.clone(), creator.clone()).into(),
			space_id,
			DID_01,
			authorization_id,
...
fn creating_a_duplicate_space_should_fail() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_err!(
			SpaceDid::create(DoubleOrigin(author.clone(), creator.clone()).into(), space_digest,),
...
fn approving_a_non_exixtent_space_should_fail() {
	...
	new_test_ext().execute_with(|| {
		assert_err!(
			SpaceDid::approve(RawOrigin::Root.into(), space_id.clone(), capacity),
...
fn archiving_a_non_exixtent_space_should_fail() {
	...
	new_test_ext().execute_with(|| {
		assert_err!(
			SpaceDid::archive(DoubleOrigin(author, creator).into(), space_id, authorization_id,),
			Error::<Test>::AuthorizationNotFound
		);
	});
}
...
fn updating_space_capacity_by_root_should_succeed() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_ok!(SpaceDid::update_transaction_capacity(
			RawOrigin::Root.into(),
			space_id.clone(),
			new_capacty
		));
...
fn updating_space_capacity_by_non_root_should_fail() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_ok!(SpaceDid::approve(RawOrigin::Root.into(), space_id.clone(), capacity));
		assert_err!(
			SpaceDid::update_transaction_capacity(
				DoubleOrigin(author.clone(), creator.clone()).into(),
				space_id,
				new_capacty,
			),
...
fn resetting_space_usage_by_root_should_succeed() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_ok!(SpaceDid::reset_transaction_count(RawOrigin::Root.into(), space_id.clone()));
	});
}
...
fn resetting_space_usage_by_non_root_should_fail() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_err!(
			SpaceDid::reset_transaction_count(
				DoubleOrigin(author.clone(), creator.clone()).into(),
				space_id,
			),
			BadOrigin
...
fn restoring_approval_of_a_space_by_root_should_succeed() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_ok!(SpaceDid::approval_revoke(RawOrigin::Root.into(), space_id.clone()));
		assert_ok!(SpaceDid::approval_restore(RawOrigin::Root.into(), space_id.clone()));
	});
}
...
fn restoring_approval_of_a_non_revoked_space_by_root_should_fail() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_err!(
			SpaceDid::approval_restore(RawOrigin::Root.into(), space_id.clone()),
...
fn add_delegate_should_fail_if_space_delegates_limit_exceeded() {
	...
	new_test_ext().execute_with(|| {
		...
		for delegate_count in 2..6 {
			assert_ok!(SpaceDid::space_delegate_addition(
				space_id.clone(),
		...
		// expected error
		assert_err!(
			SpaceDid::space_delegate_addition(
				space_id.clone(),
...
fn add_admin_delegate_should_fail_for_space_creator_delegating_themselves() {
	...
	new_test_ext().execute_with(|| {
		...
		// Adding the Admin Delegate who is the Creator of the SpaceDid should fail.
		assert_err!(
			SpaceDid::add_admin_delegate(
				DoubleOrigin(author.clone(), creator.clone()).into(),
				space_id,
...
fn add_delegator_should_fail_for_space_creator_delegating_themselves() {
	...
	new_test_ext().execute_with(|| {
		...
		// Adding the `delegator` who is the Creator of the SpaceDid should fail.
		assert_err!(
			SpaceDid::add_delegator(
				DoubleOrigin(author.clone(), creator.clone()).into(),
				space_id,
