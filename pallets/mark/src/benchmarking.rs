...
benchmarks! {
	...
	anchor {
		let content_hash: T::Hash = T::Hashing::hash(b"content hash");
	...
	verify {
		assert!(Marks::<T>::contains_key(claim_hash));
		assert_eq!(MarkModule::<T>::anchor(claim_hash), Some(Marks::<T> {
			content_hash,
			marker: delegate_acc,
	...
	verify {
		...
		assert_eq!(Marks::<T>::get(content_hash), Some(Marks::<T> {
			mtype_hash,
			marker: delegate_acc,
			delegation_id: Some(delegation_id),
			revoked: true,
