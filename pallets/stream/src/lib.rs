...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		// 	ensure!(!<Metadata<T>>::contains_key(&identifier),
		...
		// 	<Streams<T>>::insert(&identifier, StreamDetails { metadata: true,
		// ..stream_details });
		...
		// 	ensure!(!<StreamHashes<T>>::contains_key(&tx_hash),
		// Error::<T>::InvalidTransactionHash);
		...
		// 	<Streams<T>>::insert(&identifier, StreamDetails { metadata: false,
		// ..stream_details });
