...
fn check_successful_complete_creation() {
    ...
    let enc_keys = DidNewKeyAgreementKeySet::<Test>::try_from(
        vec![
            get_x25519_encryption_key(true),
            get_x25519_encryption_key(false),
        ]
        .iter()
        .copied()
        ...
        .execute_with(|| {
            ...
            assert_eq!(
                ...
                Some(generate_key_id(
                    &details.new_delegation_key.clone().unwrap().into()
                ))
            );
            assert_eq!(
                stored_did.attestation_key,
                Some(generate_key_id(
                    &details.new_attestation_key.clone().unwrap().into()
                ))
            );
            ...
            assert!(stored_did.public_keys.contains_key(&generate_key_id(
                &key_agreement_keys_iterator.next().unwrap().into()
            )));
            assert!(stored_did.public_keys.contains_key(&generate_key_id(
                &key_agreement_keys_iterator.next().unwrap().into()
            )));
...
fn check_invalid_service_id_character_did_creation() {
    let auth_key = get_sr25519_authentication_key(true);
    let alice_did = get_did_identifier_from_sr25519_key(auth_key.public());
    let new_service_details = DidEndpoint::new(
        "å".bytes().collect(),
        vec![b"type".to_vec()],
        vec![b"url".to_vec()],
    );
...
fn check_successful_authentication_key_max_public_keys_update() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert_eq!(
                public_keys.len(),
                MaxPublicKeysPerDid::get().saturated_into::<usize>()
            );
            // Check for new authentication key
            assert!(public_keys.contains_key(&generate_key_id(
                &DidVerificationKey::from(new_auth_key.public()).into()
...
fn check_successful_delegation_key_deletion() {
    ...
    ExtBuilder::default()
        ...
        .build(None)
        .execute_with(|| {
            assert_ok!(Did::remove_delegation_key(RuntimeOrigin::signed(
                alice_did.clone()
            )));
...
fn check_successful_attestation_key_update() {
    ...
    let mut old_did_details =
        generate_base_did_details::<Test>(DidVerificationKey::from(auth_key.public()));
    assert_ok!(old_did_details
        .update_attestation_key(DidVerificationKey::from(old_att_key.public()), 0u64));
...
fn check_successful_attestation_key_max_public_keys_update() {
    ...
    assert_ok!(did_details.add_key_agreement_keys(key_agreement_keys, 0u64,));
    assert_ok!(
        did_details.update_attestation_key(DidVerificationKey::from(old_att_key.public()), 0u64)
    );
...
fn check_successful_attestation_key_deletion() {
    ...
    ExtBuilder::default()
        ...
        .build(None)
        .execute_with(|| {
            assert_ok!(Did::remove_attestation_key(RuntimeOrigin::signed(
                alice_did.clone()
            )));
...
fn check_service_addition_no_prior_service_successful() {
    let auth_key = get_ed25519_authentication_key(true);
    let alice_did = get_did_identifier_from_ed25519_key(auth_key.public());
    let new_service_endpoint = DidEndpoint::new(
        b"id".to_vec(),
        vec![b"type".to_vec()],
        vec![b"url".to_vec()],
    );
...
fn check_service_already_present_addition_error() {
    let auth_key = get_ed25519_authentication_key(true);
    let alice_did = get_did_identifier_from_ed25519_key(auth_key.public());
    let service_endpoint = DidEndpoint::new(
        b"id".to_vec(),
        vec![b"type".to_vec()],
        vec![b"url".to_vec()],
    );
...
fn check_service_deletion_successful() {
    ...
    let auth_key = get_ed25519_authentication_key(true);
    let alice_did = get_did_identifier_from_ed25519_key(auth_key.public());
    let old_service_endpoint = DidEndpoint::new(
        b"id".to_vec(),
        vec![b"type".to_vec()],
        vec![b"url".to_vec()],
    );
    ...
    ExtBuilder::default()
        ...
        .with_endpoints(vec![(
            alice_did.clone(),
            vec![old_service_endpoint.clone()],
        )])
        .build(None)
        .execute_with(|| {
        ...
        .execute_with(|| {
            assert_noop!(
                Did::remove_service_endpoint(
                    ...
                    service_id
                        .try_into()
                        .expect("Service ID to delete too long")
                ),
...
fn check_call_attestation_key_successful() {
    ...
    let mut mock_did =
        generate_base_did_details::<Test>(DidVerificationKey::from(auth_key.public()));
    assert_ok!(
        mock_did.update_attestation_key(DidVerificationKey::from(attestation_key.public()), 0)
    );
