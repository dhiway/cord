...
#[test]
fn has_history_and_latest_state_event_work() {
	new_test_ext().execute_with(|| {
		let token = Ss58Identifier::to_encoded(vec![9u8; 32], 210, 4, 0).expect("encode token");
		let signer = sr25519::Pair::from_seed(&[22u8; 32]);
		let auth = make_auth(b"history-check", &signer);

		// initially no history
		assert!(!Pallet::<Test>::has_history(auth.clone(), token.clone()));
		assert!(Pallet::<Test>::latest_state_event(auth.clone(), token.clone()).is_none());

		// add two events
		let first_digest = H256::random();
		let second_digest = H256::random();
		let action: EventTypeOf = b"act".to_vec().try_into().unwrap();
		let seal1 = EventBlock { height: 3, index: 0 };
		let seal2 = EventBlock { height: 4, index: 1 };
		Pallet::<Test>::update_token_state(&token, first_digest, action.clone(), seal1).unwrap();
		Pallet::<Test>::update_token_state(&token, second_digest, action.clone(), seal2).unwrap();

		assert!(Pallet::<Test>::has_history(auth.clone(), token.clone()));
		let latest_raw =
			Pallet::<Test>::latest_state_event(auth.clone(), token.clone()).expect("latest");
		let latest = latest_raw;
		assert_eq!(latest.digest, second_digest);
		assert_eq!(latest.seal.height, 4);
	});
}

#[test]
fn recent_timeline_respects_limit_and_ordering() {
	new_test_ext().execute_with(|| {
		let token = Ss58Identifier::to_encoded(vec![10u8; 32], 220, 4, 0).expect("encode token");
		let signer = sr25519::Pair::from_seed(&[23u8; 32]);
		let auth = make_auth(b"recent", &signer);

		for i in 0..5u8 {
			let digest = H256::random();
			let seal = EventBlock { height: (10 + i as u32), index: 0 };
			let action: EventTypeOf = vec![i; 4].try_into().unwrap();
			Pallet::<Test>::update_token_state(&token, digest, action, seal).unwrap();
		}
...
fn pallet_index_views_roundtrip() {
	new_test_ext().execute_with(|| {
		...
		let name = Pallet::<Test>::pallet_name_view(make_auth(b"view-name", &pair), index)
			.expect("name view");
		assert_eq!(name, pallet_name);
	});
}
