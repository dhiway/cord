...
#[test]
fn remove_same_member_twice_is_already_queued() {
	let mut ext = new_test_ext(3);
	ext.execute_with(|| {
		remove_ok(6);
		assert_noop!(
			crate::pallet::Pallet::<Test>::remove(RawOrigin::Root.into(), 6),
...
#[test]
fn removal_guard_accounts_for_pending_queue() {
	let mut ext = new_test_ext(3);
	ext.execute_with(|| {
		remove_ok(3);
		remove_ok(6);
		assert_noop!(
			crate::pallet::Pallet::<Test>::remove(RawOrigin::Root.into(), 9),
			Err::<Test>::TooLowAuthorityCount
		);
	});
}
