...
fn commit_and_register(owner: u64, parent: Option<H256>, raw: &[u8]) -> H256 {
	...
	let commitment = Names::registration_commitment(&owner, parent, &name_label, &salt);
	assert_ok!(Names::commit(RuntimeOrigin::signed(owner), commitment));
	run_to(System::block_number() + 2);
	assert_ok!(Names::register(RuntimeOrigin::signed(owner), parent, name_label.clone(), salt));
	Names::derive_name_id(parent, &name_label)
}

#[test]
fn label_policy_is_ascii_lowercase_and_versioned() {
	new_test_ext().execute_with(|| {
		assert_eq!(Names::label_policy_version(), LABEL_POLICY_VERSION);
		assert_ok!(Names::validate_label(b"alice-42".to_vec()));
		for invalid in
			[b"Alice".as_slice(), b"-alice", b"alice-", b"alice_name", &[0xe2, 0x98, 0x83]]
		{
			assert!(matches!(
				Names::validate_label(invalid.to_vec()),
...
fn commit_reveal_binds_owner_and_age() {
	new_test_ext().execute_with(|| {
		...
		let salt = salt(b"secret");
		let commitment = Names::registration_commitment(&1, None, &label, &salt);
		assert_ok!(Names::commit(RuntimeOrigin::signed(1), commitment));
		assert_noop!(
			Names::register(RuntimeOrigin::signed(1), None, label.clone(), salt.clone()),
			Error::<Test>::CommitmentTooYoung
		);
		run_to(3);
		assert_noop!(
			Names::register(RuntimeOrigin::signed(2), None, label.clone(), salt.clone()),
			Error::<Test>::CommitmentNotFound
		);
		assert_ok!(Names::register(RuntimeOrigin::signed(1), None, label.clone(), salt));
		let name = Names::derive_name_id(None, &label);
		let record: NameRecordOf<Test> = Names::name_record(name).expect("registered");
...
fn register_rejects_empty_salt_even_when_a_commitment_exists() {
	new_test_ext().execute_with(|| {
		let label = label(b"alice");
		let empty = salt(b"");
		let commitment = Names::registration_commitment(&1, None, &label, &empty);
		assert_ok!(Names::commit(RuntimeOrigin::signed(1), commitment));
		run_to(3);
		assert_noop!(
			Names::register(RuntimeOrigin::signed(1), None, label, empty),
...
fn subname_requires_parent_authority_and_is_expiry_bounded() {
	new_test_ext().execute_with(|| {
		...
		let attacker_commitment =
			Names::registration_commitment(&2, Some(parent), &child_label, &child_salt);
		assert_ok!(Names::commit(RuntimeOrigin::signed(2), attacker_commitment));
		run_to(System::block_number() + 2);
		assert_noop!(
			Names::register(
		...
		let record = Names::name_record(child).expect("child registered");
		assert_eq!(record.depth, 1);
		assert!(record.expires_at <= Names::name_record(parent).unwrap().expires_at);
	});
}
...
fn ownership_controllers_resolvers_and_primary_are_native() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Names::add_controller(RuntimeOrigin::signed(1), name, 2));
...
fn subject_and_attestation_references_are_validated_before_mutation() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Names::set_subject(RuntimeOrigin::signed(1), name, Some(unknown_subject)),
			Error::<Test>::InvalidSubjectReference
		);
		assert_eq!(Names::name_record(name).unwrap().subject, None);

		assert_ok!(Names::set_subject(RuntimeOrigin::signed(1), name, Some(KNOWN_SUBJECT)));
		assert_noop!(
			Names::set_attestation(RuntimeOrigin::signed(1), name, Some(non_live_attestation)),
			Error::<Test>::InvalidAttestationReference
		);
		let record = Names::name_record(name).unwrap();
		assert_eq!(record.subject, Some(KNOWN_SUBJECT));
		assert_eq!(record.attestation, None);

		assert_ok!(Names::set_attestation(RuntimeOrigin::signed(1), name, Some(LIVE_ATTESTATION)));
		assert_ok!(Names::set_subject(RuntimeOrigin::signed(1), name, None));
		assert_ok!(Names::set_attestation(RuntimeOrigin::signed(1), name, None));
		let record = Names::name_record(name).unwrap();
...
fn reservations_protection_and_pause_fail_closed() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Names::set_label_protection(RuntimeOrigin::root(), protected.clone(), true));
		let salt = salt(b"secret");
		let commitment = Names::registration_commitment(&1, None, &protected, &salt);
		assert_ok!(Names::commit(RuntimeOrigin::signed(1), commitment));
		run_to(3);
		assert_noop!(
			Names::register(RuntimeOrigin::signed(1), None, protected, salt),
			Error::<Test>::ProtectedLabel
		);
		assert_ok!(Names::set_paused(RuntimeOrigin::root(), true));
		assert_noop!(
			Names::commit(RuntimeOrigin::signed(1), H256::repeat_byte(1)),
			Error::<Test>::Paused
		);
		assert_ok!(Names::set_paused(RuntimeOrigin::root(), false));
...
fn genesis_reservations_and_scoped_registrar_governance_are_enforced() {
	new_test_ext_with_names(vec![2], vec![(label(b"system"), Some(1))]).execute_with(|| {
		let reserved_label = label(b"system");
		let registration_salt = salt(b"secret");
		let attacker_commitment =
			Names::registration_commitment(&3, None, &reserved_label, &registration_salt);
		assert_ok!(Names::commit(RuntimeOrigin::signed(3), attacker_commitment));
		run_to(3);
		assert_noop!(
			Names::register(
		...
		let owner_commitment =
			Names::registration_commitment(&1, None, &reserved_label, &registration_salt);
		assert_ok!(Names::commit(RuntimeOrigin::signed(1), owner_commitment));
		run_to(5);
		...
		assert_noop!(
			Names::reserve_name(RuntimeOrigin::signed(3), None, label(b"corp"), None, None),
			Error::<Test>::NotRegistrar
		);
		assert_ok!(Names::reserve_name(
			RuntimeOrigin::signed(2),
			None,
			label(b"corp"),
			Some(1),
		...
		assert_ok!(Names::set_registrar(RuntimeOrigin::root(), 2, false));
		assert_noop!(
			Names::set_label_protection(RuntimeOrigin::signed(2), label(b"blocked"), true),
			Error::<Test>::NotRegistrar
		);
	});
}
...
fn genesis_rejects_invalid_or_duplicate_reservation_labels() {
	assert!(std::panic::catch_unwind(|| {
		new_test_ext_with_names(Vec::new(), vec![(label(b"Alice"), None)]);
	})
	.is_err());
	assert!(std::panic::catch_unwind(|| {
		new_test_ext_with_names(
			Vec::new(),
