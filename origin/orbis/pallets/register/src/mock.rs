...
use pallet_orbis_entity::{signature::SignatureVerificationError, EntityLookup};
use pallet_orbis_token::{EventBlock, Token as TokenTrait};
...
impl pallet_orbis_token::Config for Test {
...
impl Config for Test {
	type RuntimeEvent = RuntimeEvent;
	type Token = pallet_orbis_token::Pallet<Self>;
...
pub fn bind_account(account: AccountId) -> Ss58Identifier {
	...
	let token =
		<pallet_orbis_token::Pallet<Test> as TokenTrait<Test>>::build(hash.as_ref(), pallet)
