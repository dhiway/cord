...
use super::*;
use crate::register::{
	AttributeFlags, AttributeSpec, LookupSpec, RegistryPermissions, RegistryStatus,
};
use alloc::vec;
...
use pallet_orbis_entity::EntityTokenOfAccount;
...
impl<T> EntityBinder<T> for pallet_orbis_entity::Pallet<T>
where
	T: Config + pallet_orbis_entity::Config,
{
	fn bind_account(account: &T::AccountId, token: &Ss58Identifier) {
...
frame_benchmarking::benchmarks! {
	...
	remove_delegate_permissions {
		...
		RegistryDelegates::<T>::insert(&registry, &delegate_token, RegistryPermissions::ENTRY | RegistryPermissions::VIEW);
	}: _<T::RuntimeOrigin>(
		RawOrigin::Signed(caller.clone()).into(),
		registry.clone(),
	...
	verify {
		assert!(RegistryDelegates::<T>::get(&registry, &delegate_token).is_none());
	}
