...
use xcm::prelude::*;
use xcm_config::{
	FellowshipLocation, GovernanceLocation, StakingPot, UntRelayLocation,
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
	spec_name: create_runtime_str!("coretime-loom"),
	impl_name: create_runtime_str!("dw-coretime-loom"),
	authoring_version: 1,
...
parameter_types! {
	...
	pub FeeAssetId: AssetId = AssetId(UntRelayLocation::get());
...
impl_runtime_apis! {
	...
	impl xcm_runtime_apis::fees::XcmPaymentApi<Block> for Runtime {
		fn query_acceptable_payment_assets(xcm_version: xcm::Version) -> Result<Vec<VersionedAssetId>, XcmPaymentApiError> {
			let acceptable_assets = vec![AssetId(xcm_config::UntRelayLocation::get())];
			PolkadotXcm::query_acceptable_payment_assets(xcm_version, acceptable_assets)
		}

		fn query_weight_to_asset_fee(weight: Weight, asset: VersionedAssetId) -> Result<u128, XcmPaymentApiError> {
			match asset.try_as::<AssetId>() {
				Ok(asset_id) if asset_id.0 == xcm_config::UntRelayLocation::get() => {
					// for native token
	...
	impl frame_benchmarking::Benchmark<Block> for Runtime {
		...
		fn dispatch_benchmark(
			...
			use xcm_config::UntRelayLocation;

			parameter_types! {
				pub ExistentialDepositAsset: Option<Asset> = Some((
					UntRelayLocation::get(),
			...
			impl pallet_xcm_benchmarks::Config for Runtime {
				...
				fn valid_destination() -> Result<Location, BenchmarkError> {
					Ok(UntRelayLocation::get())
				}
				fn worst_case_holding(_depositable_count: u32) -> Assets {
					// just concrete assets according to relay chain.
					let assets: Vec<Asset> = vec![
						Asset {
							id: AssetId(UntRelayLocation::get()),
			...
			parameter_types! {
				pub const TrustedTeleporter: Option<(Location, Asset)> = Some((
					UntRelayLocation::get(),
					Asset { fun: Fungible(UNITS), id: AssetId(UntRelayLocation::get()) },
				));
			...
			impl pallet_xcm_benchmarks::generic::Config for Runtime {
				...
				fn transact_origin_and_runtime_call() -> Result<(Location, RuntimeCall), BenchmarkError> {
					Ok((UntRelayLocation::get(), frame_system::Call::remark_with_event { remark: vec![] }.into()))
				}
				...
				fn claimable_asset() -> Result<(Location, Location, Assets), BenchmarkError> {
					let origin = UntRelayLocation::get();
					let assets: Assets = (AssetId(UntRelayLocation::get()), 1_000 * UNITS).into();
