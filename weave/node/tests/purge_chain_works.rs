...
async fn purge_chain_works() {
	...
	let base_dir_path = format!("{}/loom", base_dir.path().display());

	let args = &["--", "-d", &base_dir_path, "--chain=loom-local"];

	common::run_node_for_a_while(base_dir.path(), args, SIGINT).await;

	assert!(base_dir.path().join("chains/local_testnet/db/full").exists());
	assert!(base_dir.path().join("loom/chains/loom_local_testnet/db/full").exists());
	...
	assert!(!base_dir.path().join("chains/local_testnet/db/full").exists());
	assert!(base_dir.path().join("loom/chains/loom_local_testnet").exists());
	assert!(!base_dir.path().join("loom/chains/loom_local_testnet/db/full").exists());
}
