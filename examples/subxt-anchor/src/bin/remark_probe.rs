...
struct Cli {
    ...
    signer: String,
    #[arg(long, default_value = "hello from remark_probe", help = "Message inserted into the remark")]
    message: String,
...
async fn main() -> color_eyre::Result<()> {
    ...
    let params = build_cord_params(
        DefaultExtrinsicParamsBuilder::<CordConfig>::new().tip(cli.tip),
    );
    ...
    println!(
        "Remark finalized (extrinsic index {:?})",
