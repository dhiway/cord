...
impl TestApi {
	...
	/// Helper function for mapping block number to hash. Use if mapping shall
	/// not fail.
	...
	fn validate_transaction(
		...
		match self.block_id_to_number(&BlockId::Hash(at)) {
			Ok(Some(number)) => {
				...
				// If there is no best block, we don't know based on which block we should
				// validate the transaction. (This is not required for this test function, but
				// in real environment it would fail because of this).
				if !found_best {
