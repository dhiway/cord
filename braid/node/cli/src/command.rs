...
impl SubstrateCli for Cli {
	...
	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		...
		let id = if id == "" {
			...
			match id {
				"dev-braid-base" | "dev-braid-plus" => "dev",
				_ => id,
			}
		};
		Ok(match id {
			...
			path => {
				...
				let chain_spec =
					Box::new(cord_braid_service::GenericChainSpec::from_json_file(path.clone())?)
						as Box<dyn cord_braid_service::ChainSpec>;
			...
			runner.sync_run(|config| {
				...
				match cmd {
					BenchmarkCmd::Pallet(cmd) => {
						...
						cmd.run_with_spec::<sp_runtime::traits::HashingFor<cord_braid_service::Block>, ()>(Some(config.chain_spec))
					},
					...
					BenchmarkCmd::Extrinsic(cmd) => {
						// ensure that we keep the task manager alive
						let partial = cord_braid_service::new_partial(&config)?;
