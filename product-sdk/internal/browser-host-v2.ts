...
export interface PrivateBrowserAuthorityResolverV2 {
  resolve(input: {
    readonly operation: Operation; readonly code: number; readonly productId: string;
...
export class PrivateDurableBrowserHostV2 {
  ...
  async invoke(
    ...
    if (upload) await this.#streamUpload(operationId, upload, signal);
    while (true) {
      ...
      if (event[3] === 3) return { error: decodeError(event[4] as WireMap) };
      return { value: decodeResult(operation, event[4] as WireMap) };
    }
  }

  async #streamUpload(operationId: Uint8Array, upload: PrivateStorageUploadV2, signal?: AbortSignal): Promise<void> {
    let sequence = 0; let length = 0n; const pending = new Set<Promise<void>>();
    for await (const source of upload.bytes) {
      ...
      for (let offset = 0; offset < source.length; offset += 262_144) {
        ...
        const digest = await this.#outbox.digest(chunk);
        const exact = encodeHostV2("ProviderTransferChunkV1", { 0: 1, 1: operationId, 2: sequence++, 3: chunk, 4: digest });
        const sending = this.#durable.sendProviderTransferChunk(exact, io(signal));
        pending.add(sending); sending.finally(() => pending.delete(sending)).catch(() => undefined);
...
export class PrivateDurableBrowserStorageV2 implements StorageV2Transport, PrivateStorageExecutorV2 {
  readonly #host: PrivateDurableBrowserHostV2;
  constructor(host: PrivateDurableBrowserHostV2) { this.#host = host; }
  ...
  start<Op extends StorageV2Operation>(intent: StorageV2Intent<Op>): StorageV2Execution<Op> {
    ...
    return { events, cancel: async () => controller.abort(), resume: (_resume: StorageV2Resume) => this.#events(intent, undefined, controller.signal) };
  }
...
async function* receiveUpload(transport: BrowserHostV2Transport, operationId: Uint8Array, declared: bigint, options: BrowserHostV2IoOptions): AsyncIterable<Uint8Array> {
  ...
  while (received < declared) {
    ...
    const digest = new Uint8Array(await globalThis.crypto.subtle.digest("SHA-256", Uint8Array.from(chunk[3]).buffer));
