...
let index = 0;
function context(method: string) { index += 1; return { request_id: `dotns-contract-${String(index).padStart(4, "0")}`, application_id: "festival", network: ORBIS_NETWORK_BINDING, consent: { scopes: [`dotns:${method}`], expires_at: 2_000, nonce: `dotns-consent-${String(index).padStart(4, "0")}` } }; }
