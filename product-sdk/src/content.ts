...
function blake2b256(input: Uint8Array): Uint8Array {
  ...
  const compress = (block: Uint8Array, last: boolean): void => {
    ...
    const work = [...state, ...BLAKE2B_IV];
    work[12] ^= BigInt(compressedBytes) & MASK_64;
    if (last) work[14] ^= MASK_64;
    ...
    for (const permutation of BLAKE2B_SIGMA) {
      mix(0, 4, 8, 12, words[permutation[0]], words[permutation[1]]);
      mix(1, 5, 9, 13, words[permutation[2]], words[permutation[3]]);
      mix(2, 6, 10, 14, words[permutation[4]], words[permutation[5]]);
      mix(3, 7, 11, 15, words[permutation[6]], words[permutation[7]]);
      mix(0, 5, 10, 15, words[permutation[8]], words[permutation[9]]);
      mix(1, 6, 11, 12, words[permutation[10]], words[permutation[11]]);
      mix(2, 7, 8, 13, words[permutation[12]], words[permutation[13]]);
      mix(3, 4, 9, 14, words[permutation[14]], words[permutation[15]]);
    }
