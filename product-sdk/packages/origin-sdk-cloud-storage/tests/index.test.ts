...
test("storage coverage and portable digest stay frozen",()=>{const pageVersion:DriveIdPage["version"]=8;assert.equal(pageVersion,8);assert.deepEqual(CLOUD_STORAGE_NATIVE_BINDINGS.runtimeApis,["StorageProviderApi.v10","DriveRegistryApi.v2","S3RegistryApi.v3"]);assert.equal(CLOUD_STORAGE_NATIVE_BINDINGS.routeCount,64);assert.equal(CLOUD_STORAGE_NATIVE_BINDINGS.appRouteCount,59);assert.equal(Object.keys(storageRequests).length+Object.keys(providerRequests).length+Object.keys(driveRequests).length+Object.keys(s3Requests).length,59);assert.equal(CLOUD_STORAGE_ADMIN_EXCLUSIONS.length,5);assert.equal(digestContent("blake2b-256",new Uint8Array()).length,32)});
