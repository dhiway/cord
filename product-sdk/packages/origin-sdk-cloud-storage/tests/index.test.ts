...
import { CLOUD_STORAGE_ADMIN_EXCLUSIONS, CLOUD_STORAGE_NATIVE_BINDINGS, createCloudStorageClient, storageRequests, providerRequests, driveRequests, s3Requests, digestContent, decimalU64, objectKey, objectKeyPrefix, type BucketId, type CloudStorageRuntimeAdapter } from "../src/index.ts";
...
test("storage coverage and portable digest stay frozen",()=>{assert.equal(CLOUD_STORAGE_NATIVE_BINDINGS.routeCount,66);assert.equal(CLOUD_STORAGE_NATIVE_BINDINGS.appRouteCount,61);assert.equal(Object.keys(storageRequests).length+Object.keys(providerRequests).length+Object.keys(driveRequests).length+Object.keys(s3Requests).length,61);assert.equal(CLOUD_STORAGE_ADMIN_EXCLUSIONS.length,5);assert.equal(digestContent("blake2b-256",new Uint8Array()).length,32)});
