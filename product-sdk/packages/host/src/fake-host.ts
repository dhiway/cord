...
export interface HostRequest {
  ...
  network: { genesis_hash: string; spec_version: number; transaction_version: number; metadata_hash: string; descriptor_contract_sha256: string; chain_spec_source_sha256: string };
...
export class FakeHost {
  ...
  grant(applicationId: string, capabilities: string[]): void { this.permissions.set(applicationId, new Set(capabilities)); }
  ...
  async execute(request: HostRequest): Promise<HostTransportResult> {
    ...
    try {
      ...
      if (!CAPABILITIES.has(request.capability) || !this.permissions.get(request.application_id)?.has(request.capability)) throw new ProductSdkError("permission_denied", "capability not granted by host");
      if (!request.consent.scope.includes(`${request.capability}:${request.method}`)) throw new ProductSdkError("permission_denied", "consent does not cover method");
      if (request.consent.expires_at <= this.now()) throw new ProductSdkError("consent_expired", "consent expired");
      if (this.consumedNonces.has(request.consent.nonce)) throw new ProductSdkError("replay", "consent nonce already used");
      this.consumedNonces.add(request.consent.nonce);
      ...
      if (this.revoked.has(request.application_id)) throw new ProductSdkError("permission_revoked", "permission revoked during signing");
    ...
    finally { if(ownsId){this.inflight.delete(request.request_id);this.finish(request.request_id,outcome)} }
  }
}
export function validateHostRequest(request: HostRequest): void {
  ...
  if(!exact(request.network,["genesis_hash","spec_version","transaction_version","metadata_hash","descriptor_contract_sha256","chain_spec_source_sha256"])||!Number.isInteger(request.network.spec_version)||!Number.isInteger(request.network.transaction_version))throw new ProductSdkError("invalid_input","invalid network envelope");
