...
pub use chain_spec::{GenericChainSpec, LoomChainSpec};
...
pub trait IdentifyVariant {
	/// Returns if this is a configuration for the `Loom` network.
...
impl IdentifyVariant for Box<dyn ChainSpec> {
	fn is_loom(&self) -> bool {
		self.id().starts_with("loom") || self.id().starts_with("c0rd")
	}
...
pub fn open_database(db_source: &DatabaseSource) -> Result<Arc<dyn Database>, Error> {
	let parachains_db = match db_source {
		...
		DatabaseSource::Custom { .. } => {
			unimplemented!("No subsystem db for custom source.");
		},
	};
	Ok(parachains_db)
}
...
{
	// Use an in-memory keystore for the configuration
	config.keystore = sc_service::config::KeystoreConfig::InMemory;

	// Directly invoke chain_ops! since the same operation is performed for all cases
