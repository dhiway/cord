...
// traits::EnsureOrigin}; pub use pallet::*;
...
// frame_system::Config>::RuntimeEvent>; 		/// Privileged origin that can add or
// remove validators. 		type AuthorityOrigin: EnsureOrigin<<Self as
// frame_system::Config>::RuntimeOrigin>; 		#[pallet::constant]
...
// 	#[pallet::storage]
// 	pub(crate) type AuthorityMembers<T: Config> = StorageValue<_,
// Vec<T::ValidatorId>, ValueQuery>;
...
// 	#[pallet::storage]
// 	pub(crate) type AuthoritiesToAdd<T: Config> = StorageValue<_,
// Vec<T::ValidatorId>, ValueQuery>;
...
// 		pub fn register(origin: OriginFor<T>, authorities: Vec<T::ValidatorId>) ->
// DispatchResult { 			T::AuthorityOrigin::ensure_origin(origin)?;
...
// 		#[pallet::call_index(1)]
// 		#[pallet::weight({100_000})]
// 		pub fn remove(origin: OriginFor<T>, authorities: Vec<T::ValidatorId>) ->
// DispatchResult { 			T::AuthorityOrigin::ensure_origin(origin)?;
...
// 		#[pallet::call_index(2)]
// 		#[pallet::weight({100_000})]
// 		pub fn go_offline(origin: OriginFor<T>, authority: T::AccountId) ->
// DispatchResult { 			let who = ensure_signed(origin)?;
...
// 		#[pallet::call_index(3)]
// 		#[pallet::weight({100_000})]
// 		pub fn go_online(origin: OriginFor<T>, authority: T::AccountId) ->
// DispatchResult { 			let who = ensure_signed(origin)?;
// 			ensure!(who == authority, Error::<T>::BadOrigin);
// 			let validator_id =
// 				T::ValidatorIdOf::convert(who).ok_or(Error::<T>::NoAssociatedValidatorId)?;
...
// 			ensure!(!<AuthorityMembers<T>>::get().contains(&a),
// Error::<T>::AuthorityAlreadyExists); 			<AuthorityMembers<T>>::mutate(|v|
// v.push(a)); 		}
// 		Ok(())
// 	}
// 	fn remove_authority_member(authorities: Vec<T::ValidatorId>) ->
// DispatchResult { 		for a in authorities {
// 			ensure!(<AuthorityMembers<T>>::get().contains(&a),
// Error::<T>::AuthorityNotFound); 			<AuthorityMembers<T>>::mutate(|vs|
// vs.retain(|v| *v != a)); 		}
// 		Ok(())
// 	}
...
// impl<T: Config> pallet_session::historical::SessionManager<T::ValidatorId,
// ()> for Pallet<T> { 	fn new_session(new_index: SessionIndex) ->
// Option<Vec<(T::ValidatorId, ())>> { 		<Self as
// pallet_session::SessionManager<_>>::new_session(new_index) 			.map(|r|
// r.into_iter().map(|v| (v, Default::default())).collect()) 	}
...
// {
// 	fn report_offence(_reporters: Vec<T::AccountId>, offence: O) -> Result<(),
// OffenceError> { 		let offenders = offence.offenders();
