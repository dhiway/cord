...
//! Provides utils for building the `Extrinsic` instances used with
//! `cord-test-runtime`.

use crate::{
	cord_test_pallet::pallet::Call as PalletCall, AccountId, Balance, BalancesCall,
...
/// Transfer used in test substrate pallet. Extrinsic is created and signed
/// using this data.
#[derive(Clone)]
pub struct Transfer {
...
impl TryFrom<&Extrinsic> for TransferData {
	...
	fn try_from(uxt: &Extrinsic) -> Result<Self, Self::Error> {
		match uxt {
			...
			Extrinsic {
				function: RuntimeCall::CordTest(PalletCall::bench_call { transfer }),
				signature: None,
			} => Ok(transfer.clone()),
			_ => Err(()),
		}
...
impl ExtrinsicBuilder {
	/// Create builder for given `RuntimeCall`. By default `Extrinsic` will be
	/// signed by `Alice`.
	...
	/// Create builder for `pallet_call::bench_transfer` from given
	/// `TransferData`.
	pub fn new_bench_call(transfer: TransferData) -> Self {
		Self::new_unsigned(PalletCall::bench_call { transfer })
	}

	/// Create builder for given `Transfer`. Transfer `nonce` will be used as
	/// `Extrinsic` nonce. Transfer `from` will be used as Extrinsic signer.
	...
	/// Create builder for `PalletCall::call_do_not_propagate` call using given
	/// parameters
	pub fn new_call_do_not_propagate() -> Self {
		Self::new(PalletCall::call_do_not_propagate {})
	}

	/// Create builder for `PalletCall::call_with_priority` call using given
	/// parameters
