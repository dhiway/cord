...
fn successful_execution_with_native_equivalent_code_gives_ok() {
	...
	t.insert(
		...
		AccountInfo::<<Runtime as frame_system::Config>::Nonce, _> {
			providers: 1,
			data: (111 * UNITS, 0u128, 0u128, 1u128 << 127),
			..Default::default()
		}
		.encode(),
	);
	...
	t.execute_with(|| {
		assert_eq!(Balances::total_balance(&alice()), 42 * UNITS - fees_after_refund);
...
fn full_native_block_import_works() {
	...
	t.execute_with(|| {
		assert_eq!(
			...
			alice_last_known_balance - 10 * UNITS - fees_after_refund,
		);
...
fn full_wasm_block_import_works() {
	...
	t.execute_with(|| {
		...
		assert_eq!(Balances::total_balance(&bob()), 179 * UNITS - 1 * fees_after_refund);
	});
}
...
fn should_import_block_with_test_client() {
	...
	let block = cord_primitives::Block::decode(&mut &block_data[..]).unwrap();
