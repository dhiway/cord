	...
	fn build(&self, nonce: u32) -> std::result::Result<OpaqueExtrinsic, &'static str> {
		with_client! {
			self.client.as_ref(), client, {
				...
				let period = cord_runtime::BlockHashCount::get().checked_next_power_of_two().map(|c| c / 2).unwrap_or(2) as u64;
...
/// Generates inherent data for benchmarking
///
...
impl ExistentialDepositProvider for Client {
	fn existential_deposit(&self) -> Balance {
		with_client! {
			self,
			_client,
			cord_runtime::ExistentialDeposit::get()
		}
	}
}
