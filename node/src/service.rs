...
pub fn new_partial(
	...
	sc_service::PartialComponents<
		...
		(
			impl Fn(cord_rpc::DenyUnsafe, sc_rpc::SubscriptionTaskExecutor) -> cord_rpc::IoHandler,
			(
...
> {
	...
	let light_deps = cord_rpc::LightDeps {
	...
	let rpc_extensions = cord_rpc::create_light(light_deps);
...
mod tests {
	...
	use codec::Encode;
	use cord_primitives::{Block, DigestItem, Signature};
	use cord_runtime::constants::{currency::PAISE, time::SLOT_DURATION};
	use cord_runtime::{Address, BalancesCall, Call, UncheckedExtrinsic};
