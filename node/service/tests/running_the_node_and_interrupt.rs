...
// 		assert!(cmd.try_wait().unwrap().is_none(), "the process should still be
// running"); 		kill(Pid::from_raw(cmd.id().try_into().unwrap()),
// signal).unwrap(); 		assert_eq!(
...
// 	assert!(first_node.try_wait().unwrap().is_none(), "The first node should
// still be running"); 	assert!(second_node.try_wait().unwrap().is_none(), "The
