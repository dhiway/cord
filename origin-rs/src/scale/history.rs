...
fn decode_dev_element(value: &Value<u32>) -> Result<DevElement> {
	use origin_primitives::view::DevElement;
	...
	match var.name.as_str() {
		...
		"Bool" => {
			Ok(DevElement::Bool(field.and_then(|f| f.as_u128()).map(|n| n != 0).unwrap_or(false)))
		},
		"U64" => {
	...
	if let ValueDef::Composite(Composite::Named(fields)) = &value.value {
		...
		for (name, field) in fields {
			match name.as_str() {
				...
				"key_utf8" => {
					key_utf8 = option_inner(field).and_then(|v| v.as_str()).map(str::to_string)
				},
